...
const TOTAL_POKEMON = 151; // Ajuste para a quantidade de Pokémon da sua geração

export default function Pokedex() {
  // Array com os IDs dos Pokémon que o usuário já capturou
  const [capturedIds, setCapturedIds] = useState([1, 4, 7, 25, 133, 150]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [allPokemon, setAllPokemon] = useState([]);

  // Gera a lista de todos os Pokémon ao abrir o modal
  useEffect(() => {
    if (isModalOpen && allPokemon.length === 0) {
      const list = Array.from({ length: TOTAL_POKEMON }, (_, i) => {
        const id = i + 1;
        return {
          id,
          // Artwork oficial da PokéAPI em alta resolução
          image: `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${id}.png`
        };
      });
      setAllPokemon(list);
    }
  }, [isModalOpen, allPokemon.length]);

  const progressPercentage = Math.round((capturedIds.length / TOTAL_POKEMON) * 100);
  ...
  return (
    <div className="pokedex-wrapper">
      ...
      <div 
        ...
        title="Clique para abrir a Pokédex completa"
      >
        <div className="progress-info">
          <span>Coleção de Pokémon</span>
          <span>{capturedIds.length} / {TOTAL_POKEMON} ({progressPercentage}%)</span>
        </div>
          ...
          <div className="modal-container" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="pokemon-grid">
              {allPokemon.map((pokemon) => {
                const isCaptured = capturedIds.includes(pokemon.id);

                return (
                  <div 
                    key={pokemon.id} 
                    className={`pokemon-slot ${isCaptured ? 'registered' : 'unregistered'}`}
                  >
                    <span className="pokemon-id">#{String(pokemon.id).padStart(3, '0')}</span>
                    <div className="sprite-box">
                      <img
                        src={pokemon.image}
                        alt={isCaptured ? `Pokémon #${pokemon.id}` : "Desconhecido"}
                        className={`pokemon-sprite ${!isCaptured ? 'silhouette' : ''}`}
                        loading="lazy"
                      />
                    </div>
                    <span className="pokemon-status">
                      {isCaptured ? 'Registrado' : '???'}
                    </span>
                  </div>
                );
              })}
